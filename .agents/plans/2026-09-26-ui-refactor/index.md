...
2. **Look & usability** — a modern UI centred on *composable plots*, per-run curves, parameter
...
- **Backend**: stop deserializing `experiment.json`/`run.json` into Python classes to *browse*
...
- **Data sources**: a `MetricSource` protocol discovers *tables* per run (all `*.csv`, not just the
...
- **Designs**: A *Cockpit* (dark IDE-like docking workspace), B *Composer* (shelf-based visual
  grammar), C *Notebook* (keyboard-first document of cells), D *Atlas* (multi-page gallery + pages).
