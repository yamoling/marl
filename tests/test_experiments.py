...
def test_unpickle_experiment_and_update_network():
    ...
    runner.train(1)
    param_values = [p.data.clone() for p in exp.algo.qnetwork.parameters()]
    ...
    assert len(list(exp.algo.qnetwork.parameters())) == len(param_values)
    for param, initial_param in zip(exp.algo.qnetwork.parameters(), param_values):
