...
export class DatasetTable {
  ...
  public static fromTestDatasets(datasets: Dataset[]) {
    return DatasetTable.fromDatasets(datasets.filter((d) => d.category === "Test"));
  }

  /** Align metric values by their actual tick, even when datasets have different ticks. @ai-edited */
  public static fromDatasets(datasets: Dataset[]) {
    const byStep = new Map<number, { step: number; [key: string]: number | null }>();
    datasets.forEach((ds) => {
      ds.ticks.forEach((step, index) => {
        let row = byStep.get(step);
        if (row == null) {
          row = { step };
          byStep.set(step, row);
        }
        row[ds.label] = ds.mean[index] ?? null;
      });
    });
    return new DatasetTable([...byStep.values()].sort((a, b) => a.step - b.step));
  }
  ...
  /** Include columns present at any tick, including when the first row is sparse. @ai-edited */
  public columns(): string[] {
    return [...new Set(this.items.flatMap((item) => Object.keys(item).filter((key) => key !== "step")))];
  }
}

export function toCSV(datasets: readonly Dataset[], ticks: number[]) {
  const csv = [];
  let firstLine = "time-step," + datasets.map((ds) => `${ds.label}-mean,${ds.label}-std,${ds.label}-ci95`).join(",");
