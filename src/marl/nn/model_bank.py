...
class MLP(LinearNN):
    ...
    def __init__(self, input_shape: tuple[int], extras_shape: Optional[tuple[int]], output_shape: tuple[int], n_neurons: int = 64):
        super().__init__(input_shape, extras_shape, output_shape)
        assert len(self.input_shape) == 1, "MLP can only handle 1D inputs"
        assert len(self.extras_shape) == 1, "MLP can only handle 1D extras"
        assert len(self.output_shape) == 1, "MLP can only handle 1D outputs"

        self.n_neurons = n_neurons

        input_size = self.input_shape[0] + self.extras_shape[0]
...
class CNN(LinearNN):
    ...
    def __init__(self, input_shape: tuple[int, int, int], extras_shape: tuple[int] | None, output_shape: tuple[int]):
        ...
        self.linear = MLP((n_features,), (self.extras_shape[0],), output_shape)
