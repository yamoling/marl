...
class CNN_DActor_CCritic(ActorCriticNN):
    ...
    def __init__(self, input_shape: tuple[int, int, int], extras_shape: tuple[int], output_shape: tuple[int], n_agents: int, n_actions: int):
        assert len(input_shape) == 3, f"CNN can only handle 3D input shapes ({len(input_shape)} here)"
        super().__init__(input_shape, extras_shape, output_shape)
        ...
        for i in range(n_agents):
            self.policyNetworks.append(torch.nn.Sequential(
    ...
    @classmethod
    def from_env(cls, env: RLEnv):
        return cls(input_shape=env.observation_shape, extras_shape=env.extra_feature_shape, output_shape=(env.n_actions,), n_agents=env.n_agents, n_actions=env.n_actions)
