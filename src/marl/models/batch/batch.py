...
class Batch(ABC):
    ...
    def for_individual_learners(self) -> "Batch":
        ...
        if self.reward_size > 1: # Need to consider this case, because multiple rewards should be at the end and dones/masks are expanded when called (so rewards needs to be as is until then)
            ...
            self.rewards = self.rewards.repeat_interleave(self.n_agents).view(*self.rewards.shape[:-1], self.n_agents, self.rewards.shape[-1])
        else:
