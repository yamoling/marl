...
class Reinforce(Agent):
    def __init__(self, gamma: float, policy_network: NN, lr=5e-4):
        ...
        self.scores = []
        self.policy_network.to(self._device, non_blocking=True)

    def choose_action(self, observation: Observation):
        with torch.no_grad():
            obs_data = torch.from_numpy(observation.data).to(self._device, non_blocking=True)
            extras = torch.from_numpy(observation.extras).to(self._device, non_blocking=True)
    ...
    def after_train_episode(self, episode_num: int, episode: Episode):
        batch = EpisodeBatch([episode]).to(self._device).for_individual_learners()
