...
def handle_distillation(episode: Episode, distil_path: pathlib.Path, extra: bool):
    if "sdt" in str(distil_path):
        ...
        if "individual" in str(distil_path):
            ...
            if len(distilled_extras) != 0:
                distilled_extras = np.transpose(np.array(distilled_extras).squeeze(), (1, 0, 2, 3))
            if e_type:
                distilled_actions = np.transpose(np.array(distilled_actions), (1, 0, 2))
            else:
                distilled_actions = np.transpose(np.array(distilled_actions), (1, 0, 2, 3))
            agent_pos = np.transpose(np.array(agent_pos), (1, 0, 2))
        else:
            distiller = SoftDecisionTree.load(distil_path)
            distilled_filters, distilled_extras, distilled_actions, agent_pos = distiller.distil_episode(
                episode, e_type
    ...
    return (
        ...
        distilled_extras,
        agent_pos,
    )  # distilled_extras is None if not applicable, filters already reformed to gameboard size
...
def main():
    ...
    if distil_path is not None:
        ...
        if experiment.env.observation_shape[1:] == (
            7,
            7,
        ):  # TODO: Hardcoded, no way to get original world size without assuming env specific type
        ...
        viewer = HeatmapXFrameViewer(
            replay.frames, episode.n_agents, agent_pos, distilled_actions, action_names, distilled_filters, distilled_extras, extras_meaning
        )
    else:
