...
def plot_target_distro(targets, path, labels):
    ...
    for cls in range(n_classes):
        ax.bar(np.arange(n_agents), counts[:, cls], bottom=bottom, label=labels[cls])
        bottom += counts[:, cls]
...
def plot_importance_with_targets(
    importance_scores,  # shape (n_samples,)
...
def get_env_infos(experiment: Experiment):
    ...
    env = experiment.env
    action_names = env.action_space.labels
...
def abstract_observation(obs, fix_feats, ag_pos):
    ...
    for agb_i in range(A):
        ...
        for beam_i, beam_key in enumerate(ag_beams):
            ...
            for ag_i in range(A):  # For each agent distance
                if ag_i != agb_i:
                    ags_to_beams[agb_i][beam_i].append(dist_to_beam(ag_pos[ag_i], beam))
                else:
                    ags_to_beams[agb_i][beam_i].append(dist_to_beam(ag_pos[ag_i], beam, True))
    ...
    for i in range(A):
        ...
        for y_g, x_g in init_gems_yx:
            if (y_g, x_g) in gems_yx:
                f += [x_g - agent_x, y_g - agent_y]  # 1 per gem: relative pos to gem
            else:
                f += [0, 0]  # If gem already taken -> very big distance
        ...
        for y, x in walls_yx:
            if x == agent_x:
                if y < agent_y:
                    temp = agent_y - y
                    if temp < wall_up or wall_up == 0:
                        wall_up = temp
                else:
                    temp = y - agent_y
                    if temp < wall_down or wall_down == 0:
                        wall_down = temp
            if y == agent_y:
                if x < agent_x:
                    temp = agent_x - x
                    if temp < wall_left or wall_left == 0:
                        wall_left = temp
                else:
                    temp = x - agent_x
                    if temp < wall_right or wall_right == 0:
                        wall_right = temp
            ...
            if ag_b_i == i:  # Own lasers
                ...
                for ag_beam in ags_to_beams[ag_b_i]:  # Only consider 1 -> lose some precision but OK, else dynamic
                    if ag_beam[i] <= 1:
                        ...
                        if math.hypot(ag_beam[ag_b_i][0], ag_beam[ag_b_i][1]) <= 1:
                            danger_and_block[1] = True  # ally can block
                        else:
                            danger_and_block[1] = False  # No one can block
...
def feature_labels(n_agents, n_gems, laser_keys):
    ...
    for j in range(n_agents - 1):
        labels += [f"Delta x to agent {j + 1}", f"Delta y to agent {j + 1}", f"Distance to agent {j + 1}"]

    # Relative distance to gems
    for g in range(n_gems):
        labels += [f"Delta x to gem {g + 1}", f"Delta y to gem {g + 1}"]

    # Relative distance to laser sources
    for l in range(len(laser_keys)):
        labels += [f"Delta x to own laser {l + 1} source", f"Delta y to own laser {l + 1} source"]

    # Relative distance to closest exit
    labels += ["Delta x to closest exit", "Delta y to closest exit", "Distance to closest exit"]
    ...
    for i, coords in enumerate(laser_keys):
        labels += [
            f"Delta x to laserbeam {i} {coords}",
