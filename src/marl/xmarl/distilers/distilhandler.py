...
class DistilHandler:
    ...
    def __init__(
        ...
        if abstract_obs and fixed_abs_features is not None:
            self.fixed_abs_features = fixed_abs_features
            self.abs_labels = [
                feature_labels(self.n_agents, len(self.fixed_abs_features[2]), self.fixed_abs_features[-1][i].keys())
    ...
    def create(
        ...
        if input_type == "abstracted_obs":
            ...
            for i in range(n_agents):
                ...
                if extras:
                    shapes_in[i] = shapes_in[i] + experiment.env.extras_shape[0]
        else:
            abstracted_obs = False
            if extras:
                input_shape = experiment.env.observation_shape[1] * experiment.env.observation_shape[2] + experiment.env.extras_shape[0] + 2
            else:
                input_shape = experiment.env.observation_shape[1] * experiment.env.observation_shape[2]

        if (
            experiment.agent.name == "DQN"
        ):  # Note: Other similar agents should work, but for our use case we'll limit to DQN and to specific transformations to the output
            # if experiment.env.reward_space.size == 1:
            if distiler != "sdt":
                raise NotImplementedError("Distilation to other model than SDT not implemented yet.")
            # Distiler may also be other models, consider simple DT and RandomForest later
            distilers = []
            if individual_agents:
                for i in range(n_agents):
                    if abstracted_obs:
                        input_shape = shapes_in[i]  # Only for abstracted
                ...
                distilers.append(distil_model)
            if abstracted_obs:
                distil_handler = DistilHandler(
                    experiment, n_runs, n_sets, epochs, distilers, distiler, extras, imp_perc, individual_agents, abstracted_obs, fix_ft
                )
            else:
                distil_handler = DistilHandler(experiment, n_runs, n_sets, epochs, distilers, distiler, extras, imp_perc, individual_agents)
        else:
    ...
    def filter_by_importance(self, inputs, outputs, ag_poses, extras, importance, percentile=50):
        ...
        if True:
            ...
            if self.extras:
                extras = np.array(filtered_extras).swapaxes(0, 1)
            # Plot action distributions after filter
            plot_target_distro(
                outputs.reshape((-1, self.n_agents, len(self.target_labels))),
                pathlib.Path(self._distilers[0].logdir, "dataset_filtered_target_distribution"),
                self.target_labels,
            )
        # else:
        ...
        for batch_idx in range(n_batches):  # Traverse batches
            ...
            for i, obs in enumerate(inputs[batch_idx]):  # Traverse batch elements
                ...
                if not self.abstract_obs:
                    ...
                    if self.extras:
                        f_obs = np.concatenate([f_obs.reshape(self.n_agents, -1), extras[batch_idx][i], ag_pos], axis=1)
                    else:
                        f_obs = f_obs.reshape(self.n_agents, -1)
                else:
                    f_obs = abstract_observation(
                        obs, self.fixed_abs_features, ag_pos
                    )  # Very specific to flattened layers. If extras also adds agent position
                    if self.extras:
                        for j in range(self.n_agents):
                            f_obs[j] += extras[batch_idx][i][j].tolist()
                t_obs[batch_idx].append(f_obs)
        if not self.abstract_obs:
            if self.individual_agents:
                return np.array(t_obs)
            else:
                return np.array(t_obs).reshape((n_batches * self.n_agents, batch_size, -1))
        else:
            return t_obs
    ...
    def run(
        ...
        if self.individual_agents:
            assert n_batches > 4, f"Given dataset too small at length: {len(inputs)}"
        else:
            assert n_batches * self.n_agents > 4, f"Given dataset too small at length: {len(inputs)}"
        ...
        if self.extras:
            extras = extras[: batch_size * n_batches].reshape(n_batches, batch_size, self.n_agents, *extras.shape[2:])
        ...
        if not self.abstract_obs:
            ...
            for i in range(self.n_agents):
                assert len(inputs[0][0][i]) == self._distilers[i].input_shape and outputs.shape[-1] == self._distilers[i].output_shape
        ...
        if self.abstract_obs:  # Put agent dim first in abstracted obs, for easier handling
            inputs_train = [
                np.array([[inputs_train[i][j][k] for j in range(batch_size)] for i in range(len(inputs_train))])
                for k in range(self.n_agents)
            ]
            outputs_train = outputs_train.transpose([2, 0, 1] + list(range(3, outputs_train.ndim)))

            inputs_validation = [
                np.array([[inputs_validation[i][j][k] for j in range(batch_size)] for i in range(len(inputs_validation))])
                for k in range(self.n_agents)
            ]
            outputs_validation = outputs_validation.transpose([2, 0, 1] + list(range(3, outputs_validation.ndim)))

            inputs_test = [
                np.array([[inputs_test[i][j][k] for j in range(batch_size)] for i in range(len(inputs_test))]) for k in range(self.n_agents)
            ]
        ...
        if self.individual_agents:
            for ag in range(len(self._distilers)):
                ...
                if not self.abstract_obs:
                    cm_dat = np.argmax(outputs_test[:, :, ag], axis=2).flatten()
                else:
                    cm_dat = np.argmax(outputs_test[ag], axis=2).flatten()

                cm_disp = ConfusionMatrixDisplay(
                    confusion_matrix=confusion_matrix(cm_dat, test_preds, labels=np.arange(len(self.target_labels))),
                    display_labels=self.target_labels,
                )  # outputs_test one-hot
                cm_disp.plot()
                plt.savefig(
                    pathlib.Path(
                        f"{self._distilers[0].logdir}",
                        f"ag{ag}_{self.dist_type}{'_extra' if self.extras else ''}{'_abstract' if self.abstract_obs else ''}_cm.png",
                    )
                )
            ...
            cm_disp = ConfusionMatrixDisplay(
                confusion_matrix=confusion_matrix(
                    np.argmax(outputs_test, axis=2).flatten(), test_preds, labels=np.arange(len(self.target_labels))
                ),
                display_labels=self.target_labels,
            )  # outputs_test one-hot
            cm_disp.plot()
    ...
    def prepare_dataset(self, device):
        ...
        plot_importance_with_targets(
            importances.flatten(),
            np.array(targets).reshape(-1, len(self.target_labels)),
            pathlib.Path(self._distilers[0].logdir, "dataset_target_importance"),
            self.target_labels,
        )
    ...
    def simulate_one_episode(self, seed: Optional[int] = None):
        ...
        extras = []
        is_finished = False
        obs, state = self._experiment.env.reset()
        while not is_finished:
            ...
            importances.append(
                qvalues.max(axis=-1) - qvalues.min(axis=-1)
            )  # Based on idea used in MAVIPER, but since out "value" function is not specified or either the mean or sum of best qvalues, this is the same
