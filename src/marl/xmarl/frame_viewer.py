...
class FrameViewer:
    ...
    def init_ctrl(self):
        ...
        self.control_fig = Figure(figsize=(3, 4))
        assert self.control_fig.canvas.manager is not None
...
class ActFrameViewer(FrameViewer):
    ...
    def init_action_plot(self):
        ...
        assert self.fig_action.canvas.manager is not None
        ...
        if self.frame_idx < self.episode_len:
            ...
            if not self.show_qvalues:
                ...
                for k in range(n_qvals):
                    ...
                    bars_pos = self.ax_action.bar(range(n_acts), pos_vals, bottom=pos_bottom, color=colours[k], label=self.qvalue_labels[k])
                    self._attach_cursor(bars_pos, [f"{self.qvalue_labels[k]}  +{v:.3f}" for v in pos_vals])
                    bars_neg = self.ax_action.bar(
                        range(n_acts),
                        neg_vals,
                        bottom=neg_bottom,
                        color=colours[k],
            ...
            self.ax_action.set_title(
                f"{'Actions' if not self.show_qvalues else 'Q-values'} - {self.selected_agent}, Frame {self.frame_idx + 1}"
            )
...
class AbstractActFrameViewer(ActFrameViewer):
    ...
    def __init__(
        ...
        actions: np.ndarray,
        action_names: list[str],
        filters_dat: list[np.ndarray],
        filters_bias: list,
        obs_dat: list,
        extras_dat: list,
        extras_meaning: list[str],
        abstract_obs_meanings: list[str],
        qvalues: np.ndarray,
        qvalue_labels: list[str],
    ):
    ...
    def init_bar_colours(self, vmin, vmax, idx):
    ...
    def init_plots(self):
        super().init_plots()
        # Main Plot Figure
        self.fig_abstract, (self.ax_split, self.ax_abstract) = plt.subplots(
            2, 1, figsize=(10, 6), gridspec_kw={"height_ratios": [1, len(self.abs_obs_dat[0][0])]}
        )  # dat has no layer dimension
        ...
        assert self.fig_abstract.canvas.manager is not None
        ...
        if self.filters_layered:  # In abstract extras are already mixed with filters (same handling)
            ...
            for j in range(self.n_agents):
                ...
                self.init_bar_colours(vmin, vmax, None)

        if self.filters_layered:
            sm = self.sm_layers[0][0]
        else:
            sm = self.sm_layers[0]
...
class HeatmapActFrameViewer(ActFrameViewer):
    ...
    def __init__(
        ...
        if self.extras:
            assert heatmap_dat.shape[:2] == (self.episode_len - 1, self.n_agents) and extras_filter.shape[:2] == (
                self.episode_len - 1,
                self.n_agents,
            )  # episode_len-1, because extra frame for final state
        else:
            assert heatmap_dat.shape[:2] == (self.episode_len - 1, self.n_agents)
    ...
    def init_plots(self):  # Override
        # Main Plot Figure
        self.fig, ((self.ax_split, temp), (self.ax_img, self.ax_bar)) = plt.subplots(
            2, 2, figsize=(10, 6), gridspec_kw={"width_ratios": [7, 3], "height_ratios": [1, 1.25 * self.heatmap_dat.shape[-2]]}
        )
        temp.axis("off")
        ...
        if self.extras:
            ...
            self.colorbar = self.fig.colorbar(sm, ax=[self.ax_img], orientation="vertical", shrink=0.85, pad=0.02)
    ...
    def init_ctrl(self):
        ...
        self.heatmap_check = CheckButtons(self.check_ax, ["Heatmap"], [True])
    ...
    def update_canvas(self):
        ...
        if self.frame_idx < self.episode_len:
            ...
            if self.show_heatmap:
                ...
                if self.heatmap_layered:
                    sm = self.sm_layers[self.selected_agent_id][self.heatmap_idx]
                else:
                    sm = self.sm_layers[self.selected_agent_id]
                self.colorbar.mappable = sm
                self.colorbar.update_normal(sm)
            ...
            if self.heatmap_layer > 1:
                if not self.combine_layers:
                    self.ax_img.set_title(
                        f"Frame {self.frame_idx + 1}/{len(self.frames)}, Filter Layer: {self.heatmap_idx + 1}/{self.heatmap_layer}"
                    )
                else:
                    self.ax_img.set_title(f"Frame {self.frame_idx + 1}/{len(self.frames)}, Filter Layer: combined")
