    ...
    def next_values(self, batch: Batch):
        ...
        if self.target_mixer is not None:
            next_values = self.target_mixer.forward(
                next_values, batch.next_states, one_hot_actions=batch.one_hot_actions, next_qvalues=next_qvalues, device=self.device
            )
    ...
    def train(self, batch: Batch) -> dict[str, Any]:
        ...
        if self.mixer is not None:
            qvalues = self.mixer.forward(
                qvalues, batch.states, one_hot_actions=batch.one_hot_actions, next_qvalues=qvalues, device=self.device
            )
