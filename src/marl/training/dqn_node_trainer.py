...
class DQNNodeTrainer(Trainer):
    ...
    def _make_graph(self, device: torch.device):
        ...
        if self.ir_module is not None:
            batch = nodes.IR(self.ir_module, batch)
        qtargets, target_parameters = self._make_targets_computation_node(batch)
        ...
        td_error = nodes.TDError(qvalues, qtargets, batch)
    ...
    def _make_qvalue_prediction_node(
        ...
        all_qvalues = nodes.QValues(self.qnetwork, batch)
        selected_qvalues = nodes.SelectedActionQValues(all_qvalues, batch)
        parameters = list(self.qnetwork.parameters())
        if self.mixer is not None:
            mixed_qvalues = nodes.QValueMixer(self.mixer, selected_qvalues, all_qvalues, batch)
            selected_qvalues = mixed_qvalues
            parameters += list(self.mixer.parameters())
        return selected_qvalues, parameters
    ...
    def _make_targets_computation_node(
        ...
        if self.double_qlearning:
            next_value = nodes.DoubleQLearning(self.qnetwork, next_qvalues, batch)
        else:
            next_value = nodes.NextValues(next_qvalues)
