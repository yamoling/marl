...
class ExpectedReturnTrainer(Trainer):
    ...
    def __post_init__(self):
        ...
        match self.optimiser_type:
            case "adam":
                self.optimiser = torch.optim.Adam(self.nn.parameters(), lr=self.lr)
            case "rms":
                self.optimiser = torch.optim.RMSprop(self.nn.parameters(), lr=self.lr)
            case other:
                raise ValueError(f"optimiser_type should either be 'adam' or 'rms' but got {other}")
        self.memory = deque[tuple[Observation, np.ndarray, float]](maxlen=self.memory_size)
    ...
    def update_episode(self, episode: Episode, episode_num: int, time_step: int) -> dict[str, float]:
        ...
        self.memory.append((initial_obs, action, total_return))
        ...
        for _ in range(self.n_epochs):
            ...
            loss = self.loss.forward(predicted_returns, returns)
