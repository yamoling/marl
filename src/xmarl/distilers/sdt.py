...
class InnerNode(nn.Module):
    ...
    def cal_prob(self, x, path_prob):
        ...
        if torch.any(torch.isnan(self.prob)) or torch.any(torch.isinf(self.prob)):
            logger.warning("prob: NaN or Inf detected!")
    ...
    def get_penalty(self):
        ...
        if torch.any(torch.isnan(penalty[0])) or torch.any(torch.isinf(penalty[0])):
            logger.warning("penalty[0]: NaN or Inf detected!")
    ...
    def get_filters(self):
        ...
        return self.fc.weight.data[
            0
        ].cpu().numpy() * self.beta.data.cpu().numpy(), self.fc.bias.data.cpu().numpy() * self.beta.data.cpu().numpy()
...
class SoftDecisionTree[B: Batch](nn.Module):
    ...
    def __init__(
        ...
        if self.agent_id is None:
            ...
            self.logdir = pathlib.Path(logdir, "distil", "individual_sdt_distil")
    ...
    def cal_loss(self, x, y):
        ...
        loss = torch.zeros(1)
        max_prob = [-1.0 for _ in range(self.batch_size)]
        ...
        for path_prob, Q in leaf_accumulator:
            if torch.any(torch.isnan(Q)) or torch.any(torch.isinf(Q)):
                logger.warning("Q: NaN or Inf detected!")
            ...
            if torch.any(torch.isnan(TQ)) or torch.any(torch.isinf(TQ)):
                logger.warning("TQ: NaN or Inf detected!")
            loss += path_prob * TQ
    ...
    def train_(self, train_data, train_targets, epoch=0):
        ...
        if self.agent_id is not None:
            logger.warning(f"Training for agent {self.agent_id}")
        self.train()
        self.define_extras(self.batch_size)
        train_log = []
        ...
        logger.info(f"Train accuracy for epoch {epoch}: {np.mean(train_log[:, 0])}\n ")
        return train_log
    ...
    def test_(self, train_data, train_targets, epoch=0, test=False):
        ...
        logger.info(f"{'Validation' if not test else 'Test'} set Accuracy: {correct}/{total_data} ({accuracy:.4f}%)\n")
    ...
    def associate_action_leaf(self, action_idx: np.ndarray, leaves: list[LeafNode], outputs: np.ndarray):
    ...
    def distil_episode_ind(self, episode: Episode, backwards: bool = False):
        ...
        if not backwards:
            ...
            for i in range(episode.episode_len):
                if not self.abstract:
                    f_obs = flatten_observation(
                        episode.all_observations[i], episode.n_agents, 1
                    )  # Gonna flatten for all, inefficient in individual
                else:
                    f_obs = abstract_observation(episode.all_observations[i], fix_ft, agent_pos[i])
                obs.append(f_obs[self.agent_id])
                if self.extras:
                    if not self.abstract:
                        leaf, path_w, path_b = self.greedy_trace(
                            np.concat([f_obs[self.agent_id].reshape(-1), episode.all_extras[i][self.agent_id], agent_pos[i]])
                        )
                    else:
            ...
            for i, act_idx in enumerate(ep_acts):
                if not self.abstract:
                    ...
                    f_obs = abstract_observation(episode.all_observations[i], fix_ft, agent_pos[i])
                obs.append(f_obs[self.agent_id])
                leaf = self.associate_action_leaf(np.array([act_idx]), leaves, outputs)[0]  # Get best leaf (only one since individual)
        ...
        if not self.abstract:
            return obs_f, obs_b, extras_f, actions_comp, agent_pos, obs, np.array(episode.all_extras)[:, self.agent_id], None
        else:
            return (
                obs_f,
                obs_b,
                None,
                actions_comp,
                agent_pos[:, self.agent_id],
                obs,
                np.array(episode.all_extras)[:, self.agent_id],
                fix_ft,
            )
    ...
    def distil_episode_comp(self, episode: Episode, backwards: bool = False):
        ...
        if not backwards:
            ...
            for i in range(episode.episode_len):
                ...
                for ag in range(episode.n_agents):
                    if extras:
                        leaf, path_w, path_b = self.greedy_trace(
                            np.concat([f_obs[ag].reshape(-1), episode.all_extras[i][ag], agent_pos[i][ag]])
                        )
                        leaves.append(leaf)
        ...
        # If applicable separate extras from board
        if extras:
            obs_f = paths_taken_weights[:, :, :, : obs_w * obs_h].reshape(
                episode.episode_len, episode.n_agents, self.max_depth, obs_h, obs_w
            )
            extras_f = paths_taken_weights[:, :, :, obs_w * obs_h :]
        else:
        ...
        return (
            ...
            actions_comp,
            agent_pos,
            flat_grid_observations,
            np.array(episode.all_extras),
            None,
    ...
    def load(filedir: str) -> "SoftDecisionTree":
        ...
        if sdt.agent_id is None:
            sdt.load_state_dict(torch.load(str(pathlib.Path(sdt.logdir, f"comp_sdt{addition}.params"))))
        else:
            sdt.load_state_dict(torch.load(str(pathlib.Path(sdt.logdir, f"ag{sdt.agent_id}_sdt{addition}.params"))))
        return sdt

    def load_best(self):
        addition = f"{'_extra' if self.extras else ''}{'_abstract' if self.abstract else ''}"
        if self.agent_id is None:
            self.load_state_dict(torch.load(str(pathlib.Path(self.logdir, f"comp_sdt{addition}.params"))))
        else:
            self.load_state_dict(torch.load(str(pathlib.Path(self.logdir, f"ag{self.agent_id}_sdt{addition}.params"))))
