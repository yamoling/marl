...
class Arguments(tap.TypedArgs):
    ...
    extras: bool = tap.arg(
        default=False, help="Will add the extras from env to the input AND! the agent's position on the grid (get when flattening)"
    )
    ...
    input: Literal["full_obs", "abstracted_obs"] = tap.arg(
        default="full_obs",
        help="Defines what type of input the distilled model should be trained on: full-obs is the standard full observation (automatically flattened to be 2D), abstracted-obs is an abstracted version of the observation containing heuristical elements of the observation (i.e.: distance from agent-0 to agent-1).",
    )  # WOuld be a good option to put "partial" view here, for the work's sake I made a dataset from an experiment which ran in partial
    ...
    individual: bool = tap.arg(
        default=False,
        help="Defines whether the distilled model should consider all agents, True: individually (1 distilled model per agent). False: as one (one distilled model, agent povs as batches).",
    )
    importance_perc: int = tap.arg(
        default=95,
        help="The value of the percentile to use when filtering the data by importance. A value of 95, keeps the 95th percentile, so the top 5% most important data.",
    )
...
def start_distillation(args: Arguments):
    ...
    if not args.individual and args.input == "abstracted_obs":
        raise Exception("Can't use abstracted obs in compounded view, abstracted view by default agent based!")
    distiler = DistilHandler.create(
        args.logdir, args.n_runs, args.n_sets, args.epochs, args.distiller, args.input, args.extras, args.individual, args.importance_perc
    )
    distiler.run()
