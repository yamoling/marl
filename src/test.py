...
def test_qmix_value():
    ...
    obs = env.reset()
    initial_qvalues = algo.compute_qvalues(obs)
    expected = torch.tensor([[6.93, 6.93], [7.92, 7.92]])
    print(initial_qvalues)
    assert torch.allclose(initial_qvalues, expected, atol=0.05)

    obs, *_ = env.step(np.array([0, 0]))
    qvalues = algo.compute_qvalues(obs)
    print(qvalues)
    expected = torch.tensor(PAYOFF_2A)
    assert torch.allclose(qvalues, expected, atol=0.05)

    env.reset()
    obs, *_ = env.step(np.array([1, 0]))
    qvalues = algo.compute_qvalues(obs)
    print(qvalues)
    expected = torch.tensor(PAYOFF_2B)
    assert torch.allclose(qvalues, expected, atol=0.05)
