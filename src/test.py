...
if __name__ == "__main__":
    logdir = "logs/test-features-static-no-agent-id-small"
    env, test_env = (
        rlenv.Builder(
            StaticLaserEnv("maps/one_laser", obs_type=ObservationType.FEATURES)
        )
        .time_limit(30)
        .build_all()
    )
    ...
    algo = (
        ...
        .gamma(0.95)
        .train_policy(
            marl.policy.DecreasingEpsilonGreedy(eps_start, eps_decrease, eps_min)
        )
        .qnetwork_default(env)
        .build()
    )
    experiment = Experiment.create(
        logdir, algo=algo, env=env, n_steps=300_000, test_interval=5000
    )
